...
import { jadwalMengajar, kelas, mapel, users } from "../db/schema.js";
...
// ============ JADWAL PER KELAS (untuk Piket/Admin) ============

export const getJadwalByKelas = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        res.status(200).json({
            success: true,
            message: `Jadwal kelas hari ${targetHari} berhasil diambil.`,
            data: jadwal
        });
    } catch (error) {
        next(error);
